...
V rámci volby uživatelského rozhraní se nabízejí dvě základní možnosti – desktopová, nebo webová aplikace. Desktopová aplikace by běžela nativně na operačním systému, což by teoreticky umožnilo vyšší výkon \cite{medium_web_vs_desktop_app}. Ta ovšem pro účely mého projektu není klíčová, neboť zpracovávaný objem dat nevyžaduje vysokou výkonnost. Zvolil jsem proto webovou aplikaci, která v mém případě běží lokálně, čímž odpadají náklady na provoz externího serveru.

Hlavní přínos webového rozhraní spočívá v tom, že výsledky si může zobrazit jakýkoliv uživatel v rámci dané sítě. Zároveň je toto prostředí pro uživatele přirozené a intuitivní – například prokliknutí z aplikace přímo na web sázkové kanceláře působí v prohlížeči intuitivně. Často zmiňovanou nevýhodou webových aplikací je nižší bezpečnost oproti aplikacím nativním \cite{medium_web_vs_desktop_app}, což se však mého projektu netýká, jelikož aplikace nepracuje se senzitivními údaji.

Jakožto vývojové prostředí jsem tedy zvolil ASP.NET Core. Jedná se o populární open-source framework, který je pro tento typ aplikací ideální. 

Co se struktury rozhraní týče, tak na hlavní stránce uživatel vidí všechny zápasy s primárními kurzy seřazené od nejvýdělečnějších kurzů, respektive od nejméně prodělečných. Daný kurz si člověk může rozkliknout, čímž se dostane na stránku daného zápasu dané sázkové kanceláře. Také si může rozkliknout zápas jako takový, čímž se dostane do Countru, který umožňuje spočítat, kolik na jaký kurz vsadit, aby byla sázka vždy stejně výnosná. 
...
Příklad spuštěné webové aplikace lze vidět na Obr.  \ref{fig:web_example}. Uživatel si může rozkliknout jednotlivé zápasy, čímž se dostane na stránku Counter, kde si může spočítat, kolik na jaký kurz vsadit, aby vždy vyhrál stejně. (viz. Obr. \ref{fig:counter}) Rozkliknutím kurzu se otevře stránka daného zápasu, pro který se kurz vypsal. 

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{img/Main.png} 
	\caption{Hlavní stránka webové aplikace.}
	\label{fig:web_example}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{img/Counter.png} 
	\caption{Stránka Counter pro rozkliknutý zápas.}
	\label{fig:counter}
\end{figure}
