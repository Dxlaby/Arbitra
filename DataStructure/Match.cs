...
namespace Arbitra.DataStructure
{
    public class Match
    {
        ...
        public List<Event> SplitToEvents()
        {
            ...
            DateTime pragueDate = TimeZoneInfo.ConvertTimeFromUtc(Date, pragueZone);
            List<Event> events = Odds.SplitToEvents(Name, RecognitionTeamName1, RecognitionTeamName2, pragueDate);
            return new List<Event>(events);
        }

        public bool IsSame(Match otherMatch)
        {
            if ( (RecognitionTeamName1.Contains(otherMatch.RecognitionTeamName1) || otherMatch.RecognitionTeamName1.Contains(RecognitionTeamName1))
                 && (RecognitionTeamName2.Contains(otherMatch.RecognitionTeamName2) || otherMatch.RecognitionTeamName2.Contains(RecognitionTeamName2)) 
        ...
        {
            //here is the core of how to find if two events from different betting shops are the same events
